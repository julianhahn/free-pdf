...
/// A sheet with the table stuck to it, the way a lit table sticks to it: a fold
/// catching the light in a bright spur that runs off to the right border, and a
/// sheen up the right side of the frame, widest in the bottom right corner where it
/// touches both borders.
///
/// Neither is paper and both are bright, so neither is background, and where the
/// spur touches the sheet all three are one shape - whose bottom right corner is
/// the corner of the *frame*. The spur is three pixels thick and the sheen tapers
/// to nothing where the two meet, which is the one thing that tells them from the
/// sheet: the sheet is hundreds of pixels wide in every direction.
///
/// 400 pixels wide on purpose, which is the size the search itself works at, so one
/// pixel here is one pixel there and the shapes are the shapes the search sees.
/// Returns its own four corners.
fn sheet_with_the_table_lit_beside_it() -> (DynamicImage, [core_engine::Point; 4]) {
    let corner = |x: f32, y: f32| core_engine::Point { x, y };
    let corners = [
        corner(90.0, 60.0),
        corner(310.0, 60.0),
    ...
    let mut img = RgbImage::from_pixel(400, 500, Rgb([28, 22, 18])); // dark table
    for y in 60..=440 {
        let share = (y - 60) as f32 / 380.0;
        let left = 90.0 + (60.0 - 90.0) * share;
        let right = 310.0 + (350.0 - 310.0) * share;
        for x in left.ceil() as u32..=right.floor() as u32 {
    ...
    for y in 300..303 {
        for x in 330..400 {
            img.put_pixel(x, y, Rgb([245, 245, 245])); // the fold, blown out
        }
    }
    for y in 300..500 {
        for x in 399 - (y - 300) / 5..400 {
            img.put_pixel(x, y, Rgb([200, 200, 200])); // sheen on the table
        }
    }
...
#[test]
fn a_lit_table_beside_the_sheet_is_not_part_of_it() {
    let (photo, corners) = sheet_with_the_table_lit_beside_it();

    let sheet = find_paper(&photo).expect("no sheet was found");

    // Counting the spur and the sheen in puts the bottom right corner on (399, 499),
    // the corner of the frame, instead of on (350, 440), the corner of the paper.
    for (found, expected) in sheet.corners().iter().zip(corners) {
        let off_by = (found.x - expected.x).hypot(found.y - expected.y);
        assert!(
            off_by < 6.0,
            "corner {found:?} is {off_by:.1} pixels away from {expected:?}"
    ...
    // And it would say the paper leaves the frame, which stops the straightening
    // this photo is a candidate for.
