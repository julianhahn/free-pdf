...
| 3 | Rebuild the iPhone client against the approved components and the flows - which is also when the sixteen capabilities get their controls. Underway, screen by screen: [`TASKS.md`](./TASKS.md) 14 to 19, of which 14 to 18 are built. Next is 19, the done screen. Adjust brought a fourth C function with it, `freepdf_suggest_adjustments`, so every control opens on what the engine would have chosen. What it does not yet do is remember: a crop, a turn and a moved slider die at the next Apply, which is what [`TASKS.md`](./TASKS.md) 20 to 23 are for. Grey still greys the screen only; task 23 makes it a real run. | client agent |
