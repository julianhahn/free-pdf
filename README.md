...
- **Some sides bow more than the millimetre a page may lose**, and row 8 above counts them.
  After task 36 the desk that is left sits in a wedge near one end of a long side, not in a
  band along it - up to four millimetres on `sheen_7`'s top. A straight side cannot follow
  that bow, however
  it is placed, and the allowance cannot grow: the millimetre is Julian's. Raising the cap
  does not even work in the small - at 20 pixels `extra_4`'s bottom got worse, -27 to -60,
  because moving a side slides the nine places it is read at along the edge. The way up is a
  corner of its own for each end, or four sides that may bend, and neither is worth building
  until he looks at a page and says the wedge shows. The `ponytail:` note on
  `sides_read_again_in_the_photo` carries the arithmetic.
