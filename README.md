...
| 16 | **open, next** ([`TASKS.md`](./TASKS.md) 42) - Julian, on a phone on 2026-08-24: a first tap on the empty border of a button, inside its outline but off its label, usually does nothing and he has to tap again. Not diagnosed yet, and the task says to find out which of three causes it is before changing a line - a dead hit area, a scroll view taking the first touch, or a transparent layer over the button - because they have different fixes. Whatever it turns out to be, it is fixed in the one place every button routes through. | client agent |
...
| 15 | **done** on 2026-08-24 ([`TASKS.md`](./TASKS.md) 36, fourth round) - Julian saw a wedge of desk at the BOTTOM RIGHT CORNER of a page on his phone, the class row 10 parked: a left or right side, worst at its very end. Settled on HIS photograph, read off the device with `devicectl` (the traps in that are written into [`ios/AGENTS.md`](./ios/AGENTS.md)). **Two caps gate each other and neither alone does anything**: `PLACES_READ_AGAIN` decides where a side is READ, `MOST_INWARD` how far it may be MOVED, so a corner no reading finds cannot be reached however high the cap, and a corner a reading finds cannot be reached if the cap refuses. Swept one at a time each looked useless - at ten, spreading the readings over 9, 13, 19 and 29 places left the run of places outside the paper at twenty-one every time; at nine readings, thirty to forty moved his corner from -22 to -21. **Swept together, 29 places and `MOST_INWARD = 60` take his corner from -42 to -6 and its run from 21 places to 2**, and a hundred buys nothing more. Over the twelve: 43 of 48 sides within -3 against 37, mean middle 17.4 pixels against 7.2, worst 50 against 12 - a page pays about 1.5 mm of margin and 4.3 mm at worst. On the photograph he took after it shipped, three sides read -1 to -3 with runs of one or two places and he called it good. `cargo test --workspace` 69 green. **Two prices, both written at the constants**: the +12 middle rows 8 and 9 held is given up, because a straight side pays at its middle for what it buys at its end; and a misread place may now cut 61.5 pixels rather than 11.5, so `MOST_INWARD` has nearly stopped being a guard - a page missing a line of writing is what to look for, and the way back is a guard that knows a misread from a bow, not a lower number. | engine agent |
